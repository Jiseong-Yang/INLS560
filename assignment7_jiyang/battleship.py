...
class HumanPlayer:
    ...
    def __init__(self, player_name):
        ...
        # Variables to keep track of the game statistics.
        self.hit_counter = 0
    ...
    def position_boat(self, boat):
        ...
        # Ask the user for the top-left position of the boat, handling the possible errors.
        position = None
    ...
    def take_turn(self):
        ...
        print("You have attacked ", str(self.attack_counter), " times, with ", str(self.hit_counter), " hits and ", \
              str(self.attack_counter - self.hit_counter), " misses.")
        ...
        # Perform attack and add 1 to the attack counter.
        hit_flag = self.opponent.board.attack(x, y)
        self.attack_counter += 1

        # If hit, print out the message and add 1 to the hit counter.
        if hit_flag:
...
class ComputerPlayer:
    ...
    # Link the player to its opponent.
    def set_opponent(self, opponent):
        self.opponent = opponent
    ...
    def position_fleet(self):
        ...
        # Notify that the computer has positioned its fleet.
        print("The computer has positioned its fleet.")

    # Positions a single boat.  Helper method for positionFleet
    def position_boat(self, boat):
        # Randomly pick the orientation of a boat.
        orientation = rd.choice(["v", "h"])

        # Randomly pick the top-left position of the boat.
        position = None
        while position is None:
            try:
                x_random = rd.randint(0, 9)
                y_random = rd.randint(0, 9)
                position = str("%d,%d") % (x_random, y_random)
                coords = position.split(",")
                x = int(coords[0])
                y = int(coords[1])
                boat.set_orientation(orientation)
            ...
            # The computer will validate the randomly produced inputs without producing any messages which are necessary
    ...
    def take_turn(self):
        ...
        # Perform attack and print out the message.
        hit_flag = self.opponent.board.attack(x, y)
        if hit_flag:
            print(self.player_name, "attacked", "("+str(x)+","+str(y)+")", "and hit a boat!")
        else:
            print(self.player_name, "attacked", "("+str(x)+","+str(y)+")", "and missed.")
